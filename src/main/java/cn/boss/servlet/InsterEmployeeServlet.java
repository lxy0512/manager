...
public class InsterEmployeeServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ei.setSd_department(request.getParameter("department"));
		EmployeeServiceImpl esi = new EmployeeServiceImpl();
		int count = esi.saveEmployee(ei);
		System.out.println(count);
		response.sendRedirect("EmployeeServlet?index=1&pg=1");
//		if (count > 0) {
//			request.setAttribute("position", ei.getC_compile());
//			request.getRequestDispatcher("EmployeeServlet?index=1").forward(request, response);
//		}else {
