...
public class EmployeeDaoImpl implements IEmployeeDao{
	...
	public List<Employee_Info> listSomeEmployee(String state, int index, int pageSize) {
		...
		try {
			String sql = "select ey_id,ey_name,ey_age,ea_sex,ey_tel,ey_idcard,sd_department,ey_address,c_compile from employee_info,employee_sex,storage_department ,compile where c_compile=? AND ea_id=ey_sex and sd_id=ey_department and c_id=ey_compile limit ?,?;";
	...
	public Employee_Info listSomeEmployee(int id) {
		PreparedStatement pstat = null;
		ResultSet rs = null;
		Employee_Info ei = null;
		try {
			String sql = "select ey_id,ey_name,ey_age,ea_sex,ey_tel,ey_idcard,sd_department,ey_address,c_compile from employee_info,employee_sex,storage_department ,compile where ey_id=? AND ea_id=ey_sex and sd_id=ey_department and c_id=ey_compile";
			pstat = BaseDao.getConn().prepareStatement(sql);
			pstat.setInt(1, id);
			rs = pstat.executeQuery();
			if (rs.next()) {
				ei = new Employee_Info();
				ei.setEy_id(rs.getInt("ey_id"));
				ei.setEy_name(rs.getString("ey_name"));
				ei.setEy_age(rs.getInt("ey_age"));
				ei.setEa_sex(rs.getString("ea_sex"));
				ei.setEy_tel(rs.getString("ey_tel"));
				ei.setEy_idcard(rs.getString("ey_idcard"));
				ei.setSd_department(rs.getString("sd_department"));
				ei.setEy_address(rs.getString("ey_address"));
				ei.setC_compile(rs.getString("c_compile"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
	...
	public int getTotalCount(String state) {
		...
		int count = -1;
		try {
			String sql = "select count(1) from employee_info,compile where ey_compile=c_id AND c_compile=?;";
			pstat = BaseDao.getConn().prepareStatement(sql);
			pstat.setString(1, state);
			rs = pstat.executeQuery();
			rs.next();
